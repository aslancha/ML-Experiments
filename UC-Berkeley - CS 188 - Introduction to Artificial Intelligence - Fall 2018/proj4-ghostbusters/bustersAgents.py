...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        mlPos = ()
        mlPosDict = util.Counter()
        for livGhostPosDist in livingGhostPositionDistributions:
            mlPos = max(livGhostPosDist, key=livGhostPosDist.get)
            distance = self.distancer.getDistance(pacmanPosition, mlPos)
            mlPosDict[mlPos] = distance
        mlPos = min(mlPosDict, key=mlPosDict.get)
        distance = mlPosDict[mlPos]
        
        
        actions = util.Counter()
        for action in legal:
            successorPos = Actions.getSuccessor(pacmanPosition, action)
            distance = self.distancer.getDistance(successorPos, mlPos)
